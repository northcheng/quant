  ...
  try:
    ...
    if 'adx' in trend_indicators:
      ...
      conditions = {
        ...
        'wave': '(-1 < adx_value_change < 1) or (-1 <= adx_direction_day <= 1 and -5 < adx_direction < 5)'} 
      values = {
        'up': 'u', 
        'down': 'd',
        'wave': 'n'}
  ...
  try:
    ...
    if 'candle' in perspective:
      ...
      if 'windows' > '':
        ...
        df['反弹_signal'] = ''
        conditions = {
          'rebound': '(candle_gap != 2 and window_position_days > 2) and ((相对窗口位置 == "up") and (previous_相对窗口位置 == "mid_up" or previous_相对窗口位置 == "mid" or ((previous_相对窗口位置 == "mid_down" or previous_相对窗口位置 == "out")and previous_candle_color == 1)))', 
          'hitpeak': '(candle_gap !=-2 and window_position_days <-2) and ((相对窗口位置 == "down") and (previous_相对窗口位置 == "mid_down" or previous_相对窗口位置 == "mid" or ((previous_相对窗口位置 == "mid_up" or previous_相对窗口位置 == "out") and previous_candle_color == -1)))'} 
        values = {
          'rebound': 'u',
          'hitpeak': 'd'}
        ...
        df['突破_signal'] = ''
        conditions = {
          'break_up': '((candle_gap != 2 and previous_window_position_days < 0) and ((candle_color == 1 and (相对窗口位置 == "out" or (entity_trend != "d" and candle_entity_middle > candle_gap_top))) or (相对窗口位置 == "up")))',
          'break_down': '((candle_gap != -2 and previous_window_position_days > 0) and ((candle_color ==-1 and 相对窗口位置 == "out" or (entity_trend != "d" and candle_entity_middle < candle_gap_bottom)) or (相对窗口位置 == "down")))'}
        values = {
          'break_up': 'u', 
          'break_down': 'd'}
      ...
      for col in [
        ...
        '启明黄昏_signal', '窗口_signal', '突破_signal', '反弹_signal', 'prev_突破_trend', 
        
        # '相对窗口位置', 
        'window_upper_limit', 'window_lower_limit', 'window_position_days', 'previous_window_position_days', 'previous_相对窗口位置', 'previous_candle_color', 'next_突破_trend',
...
def calculate_ta_signal(df):
  ...
  df['trend'] = ''
  df['signal'] = ''
  df['entity_mean'] = (df['Open'] + df['Close']) / 2
  df['ichimoku_mean'] = (df['tankan'] + df['kijun']) / 2
  df['entity_signal'] = cal_crossover_signal(df, fast_line='entity_mean', slow_line='ichimoku_mean', result_col='entity_signal', pos_signal=1, neg_signal=-1, none_signal=0)
  df['entity_signal'] = sda(df['entity_signal'], zero_as=1)

  # buy conditions
  buy_conditions = {

    # # version 1 - started 2021-12-16
    ...
    'adx': '(adx_direction > 0)',
    # 'candle': '(5 >= entity_signal > 0 and candle_color == 1)',
    'others': '((trend_idx > 1) or ((0 < 突破_day < 3) or (0 < 窗口_day < 3)))',
  ...
  sell_conditions = {
    ...
    'adx': '(adx_direction < 0)',
    'adx-trend_idx': '(adx_direction < 0 and trend_idx < 0)',
    # 'other': '(trend_idx <= 0)'
    # 'candle': '(-5 <= entity_signal < 0 and candle_color == -1)',
    # 'others': '((tankan_kijun_signal < 0) or (Close < ichimoku_mean))',
  ...
  wave_conditions = {
    ...
    # 'waving':       '((-10 <= prev_adx_extreme <= 10) and (adx_strong_day < 0))'
...
def plot_adx(df, start=None, end=None, use_ax=None, title=None, plot_args=default_plot_args):
  ...
  green_mask = ((df.adx_direction > 0)) # | (df.next_adx_diff_ma_change > 0)
  red_mask = ((df.adx_direction < 0)) # | (df.next_adx_diff_ma_change < 0)
...
def plot_multiple_indicators(df, args={}, start=None, end=None, save_path=None, save_image=False, show_image=False, title=None, width=25, unit_size=2.8, wspace=0, hspace=0.015, subplot_args=default_plot_args):
  ...
  if new_title is None:
    new_title == ''
  fig.suptitle(f'{title} - {new_title}({close_rate}%){desc}', x=0.5, y=0.99, fontsize=22, bbox=dict(boxstyle="round", fc=title_color, ec="1.0", alpha=0.05))

...
def calculate_ta_trend(df, trend_indicators, volume_indicators, volatility_indicators, other_indicators, signal_threshold=0.001):
  ...
  try:
    ...
    if 'ichimoku' in trend_indicators:
      ...
      wave_idx = df.query(f'((kijun_rate == 0) and (tankan < kijun))').index
    ...
    if 'renko' in trend_indicators:
      ...
      wave_idx = df.query('(renko_brick_length >= 20 and renko_brick_length>3*renko_duration_p1)').index
...
def calculate_ta_signal(df):
  ...
  sell_conditions = {
    ...
    # 'overall trend is down': '(trend_idx < -1 and up_trend_idx == 0)',
    # 'price went down through brick or waving': '(renko_trend == "d" or kijun < renko_h)', # or (renko_brick_length > 3*renko_duration_p1 and tankan < kijun and kijun < renko_l) 
...
def add_renko_features(df, brick_size_factor=0.1, merge_duplicated=True, cal_signal=True):
  ...
  for idx, row in renko_df.iterrows():
    if prev_idx is not None:
      renko_df.loc[idx, 'renko_series'] = (renko_df.loc[prev_idx, 'renko_series'] + renko_df.loc[idx, 'renko_trend']*int(renko_df.loc[idx, 'renko_brick_number']))[-series_len:]
    prev_idx = idx

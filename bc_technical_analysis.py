...
def calculate_ta_score(df):
  ...
  df = cal_score(df=df, condition_dict=mid_trend_condition_dict, up_score_col='mid_trend_score', down_score_col='mid_trend_score')
  ...
  position_condition_dict = {
    '+tankan':        [1, '', '(tankan_day > 0)'],
    '-tankan':        [-1, '', '(tankan_day < 0)'],

    '+kijun':         [1, '', '(kijun_day > 0)'],
    '-kijun':         [-1, '', '(kijun_day < 0)'],

    '+kama_fast':     [1, '', '(kama_fast_day > 0)'],
    '-kama_fast':     [-1, '', '(kama_fast_day < 0)'],

    '+kama_slow':     [1, '', '(kama_slow_day > 0)'],
    '-kama_slow':     [-1, '', '(kama_slow_day < 0)'],
  }
  df = cal_score(df=df, condition_dict=position_condition_dict, up_score_col='position_score', down_score_col='position_score')

  # major score and description
  major_condition_dict = {
    '+adx':            [1, '', '(0 < adx_day <= 5)'],
    '-adx':            [-1, '', '(0 > adx_day >= -5)'],

    '+ichimoku':       [1, '', '(0 < ichimoku_day <= 5)'],
    '-ichimoku':       [-1, '', '(0 > ichimoku_day >= -5)'],
...
def calculate_ta_signal(df):
  ...
  df = assign_condition_value(df=df, column='mid_trend', condition_dict=conditions, value_dict=values, default_value='n')

  # mid term trend
  df['overall_trend'] = ''
  conditions = {
    'up':     'mid_trend == "u" and short_trend == "u"', 
    'down':   'mid_trend == "d" or short_trend == "d"',
  } 
  values = {
    'up':     'u', 
    'down':   'd',
  }
  df = assign_condition_value(df=df, column='overall_trend', condition_dict=conditions, value_dict=values, default_value='n')
  ...
  df['signal'] = ''
  conditions = {
    'buy_1':          '(overall_trend == "u") and (adx_strong_day > 0)',
    'buy_2':          '(overall_trend == "n") and (trigger_score >= 2)',
  ...
  none_potential_conditions = {
    ...
    # '价格过高':       f'label == "potential" and (adx_day > 5) and (kama_fs_day >= 10 and ichimoku_fs_day >= 10 and Close > kama_fast and Close > tankan)',
    ...
    '趋势下降':       f'label == "potential" and (ichimoku_distance < 0 and kama_distance < 0 and tankan_rate <= 0 and kama_fast_rate <= 0) or (major_score <= 0)',
...
def plot_candlestick(df, start=None, end=None, date_col='Date', add_on=['split', 'gap', 'support_resistant', 'pattern'], use_ax=None, ohlcv_col=default_ohlcv_col, color=default_candlestick_color, plot_args=default_plot_args, interval='day'):
  ...
  if 'support_resistant' in add_on:
    ...
    if not np.isnan(resistant):
      ...
      print(max_x, resistanter, y_text_resistant)
...
def plot_multiple_indicators(df, args={}, start=None, end=None, interval='day', save_path=None, save_image=False, show_image=False, title=None, width=25, unit_size=3, wspace=0, hspace=0.015, subplot_args=default_plot_args):
  ...
  # score description
  score_desc = f'Signal {df.loc[df.index.max(), "signal_day"]} | Potential {df.loc[df.index.max(), "label_score"]} | Major {df.loc[df.index.max(), "major_score"]} | Trigger {df.loc[df.index.max(), "trigger_score"]} | Score {df.loc[df.index.max(), "score"]}'

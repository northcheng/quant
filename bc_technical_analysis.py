...
def calculate_ta_static(df, indicators=default_indicators):
  ...
  try:
    ...
    if target_indicator in indicators['trend']:
      ...
      # tankan-kijun signal
      conditions = {
        'up': 'ichimoku_distance < 0',
        'down': 'ichimoku_distance > 0',
  ...
  try:
    ...
    if 'candle' in perspective:
      ...
      for col in all_candle_patterns:
        ...
        df[f'{col}_day'] = sda(series=df[f'{col}_day'], zero_as=None)

      # # multi-candle signals
      # for t in [
      # '位置', 'entity', 'shadow', 'upper_shadow', 'lower_shadow', '窗口', '反弹', '突破', 
...
def plot_multiple_indicators(df, args={}, start=None, end=None, save_path=None, save_image=False, show_image=False, title=None, width=25, unit_size=2.8, wspace=0, hspace=0.015, subplot_args=default_plot_args):
  ...
  linear_desc = df.loc[df.index.max(), "linear_description"]
